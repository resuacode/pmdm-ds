...
## Ejemplo de ciclo de vida de una actividad (Compose)

En este curso las apps usan Jetpack Compose. La actividad suele extender `ComponentActivity` y montar la UI con `setContent { }` (no con layouts XML ni `setContentView`).
...
Los callbacks del ciclo de vida siguen siendo los mismos. En Compose, si necesitas reaccionar a cambios de ciclo de vida desde un composable (por ejemplo liberar un reproductor), puedes combinar `LocalLifecycleOwner` con `DisposableEffect` o APIs de Lifecycle.

:::info[Views / XML]
`AppCompatActivity` + `setContentView(R.layout…)` es el patrón clásico con Views. Lo verás en código antiguo, pero en este módulo la UI se construye con Compose.
:::
