...
La arquitectura MVVM es una arquitectura popular en Android y encaja bien con Jetpack: **ViewModel**, **StateFlow** / Flow, Room, DataStore, etc. En este curso expondremos el estado de la UI con **StateFlow** (y lo observaremos desde Compose con `collectAsState` / `collectAsStateWithLifecycle`).

:::tip[LiveData]
`LiveData` sigue existiendo y aparece en muchos tutoriales antiguos. Es una opción válida, pero en este módulo priorizamos **StateFlow** junto a corrutinas.
:::   
