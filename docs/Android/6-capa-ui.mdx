...
4. **Conectar el ViewModel con la View**: En Compose obtén el ViewModel con la función `viewModel()` (`androidx.lifecycle.viewmodel.compose`) y observa su estado (por ejemplo un `StateFlow`) con `collectAsStateWithLifecycle()` o `collectAsState()`.
