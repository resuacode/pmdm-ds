...
Para llamar a una `suspend` desde un `main` de consola o Kotlin Playground puedes usar `runBlocking` (bloquea el hilo hasta terminar). **En una app Android no uses `runBlocking` en el hilo principal**: lanza corrutinas con `viewModelScope`, `lifecycleScope` o `LaunchedEffect`.

```kotlin
// Solo para demos de consola / Playground
fun main() {
    runBlocking {
        Timer.start { count ->
            println("Count: $count")
...
class TimerViewModel : ViewModel() {
    fun start() {
        viewModelScope.launch {
            Timer.start { count -> /* actualizar StateFlow */ }
        }
    }
}
```

En el ejemplo de consola, `runBlocking` espera a la corrutina. En Android, el alcance del ViewModel cancela el trabajo cuando el ViewModel se destruye. 
...
- **Retrofit**: Biblioteca de cliente HTTP para Android y Java que facilita la conexión a servicios web RESTful. Es la opción que usaremos en el curso.

- **Ktor**: Framework de cliente y servidor web en Kotlin.

- **Volley**: Biblioteca de red más antigua de Google; la verás en material legado, pero **no** es el camino del curso.

Vamos a centrarnos en **Retrofit**. 
...
1. Abre `gradle/libs.versions.toml` (o el `build.gradle.kts` del módulo `app` si aún no usas catalog).
2. Añade Retrofit (y el convertidor que vayas a usar: scalars al principio, Gson o kotlinx.serialization después).
3. Pulsa **Sync Now**.
4. Añade el permiso `INTERNET` en el manifest (apartado anterior).
5. Si Sync falla, consulta la [guía de problemas Gradle](./12-gradle-troubleshooting.mdx).

```kotlin title="build.gradle.kts (módulo app) — ejemplo directo"
dependencies {
    // Comprueba en Maven Central la última estable; el número es orientativo
    implementation("com.squareup.retrofit2:retrofit:2.11.0")
    implementation("com.squareup.retrofit2:converter-scalars:2.11.0")
    // Más adelante: converter-gson o kotlinx.serialization
}
```

```toml title="O con version catalog"
# [versions] retrofit = "2.11.0"
# [libraries] retrofit = { module = "com.squareup.retrofit2:retrofit", version.ref = "retrofit" }
```

Estas líneas importan Retrofit y el convertidor de escalares (respuestas como `String`). Para JSON estructurado verás Gson o kotlinx.serialization en la unidad siguiente.

:::warning[Si Sync falla con Retrofit]
- “Could not resolve”: red del aula o versión mal escrita → [guía Gradle](./12-gradle-troubleshooting.mdx).
- Asegúrate de tener `google()` y `mavenCentral()` en los repositorios.
:::
...
Añade el convertidor de Gson (incluye Gson). Versión orientativa; acepta la que sugiera el IDE si es estable:
...
Si Sync falla, ver [guía Gradle](./12-gradle-troubleshooting.mdx).
:::
...
- [Documentación de Volley](https://developer.android.com/training/volley) *(legado; no es el camino del curso)*
- [Guía de dependencias y Sync en este curso](./12-gradle-troubleshooting.mdx)
