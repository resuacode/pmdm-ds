...
1. Mira la versión de **Kotlin** de tu proyecto (`libs.versions.toml` o plugins del root).
2. Añade el plugin de serialization con **la misma versión de Kotlin** (no uses un pin antiguo tipo `1.8.10` si tu Kotlin es 2.x).
3. Añade `kotlinx-serialization-json` en el módulo `app`.
4. Si usas Retrofit, añade el converter de Jake Wharton y OkHttp.
5. **Sync Project with Gradle Files**.
6. Si falla, [guía de problemas Gradle](./12-gradle-troubleshooting.mdx) (sobre todo Kotlin ↔ plugin serialization).

```kotlin title="build.gradle.kts (módulo app)"
plugins {
    // Misma versión que el plugin de Kotlin del proyecto
...
En el version catalog / plugins del proyecto, alinea el plugin de serialization con la versión de Kotlin del wizard (p. ej. 2.0.x), sin fijar a mano una Kotlin 1.8 antigua.

:::tip[Versiones]
Los números de `kotlinx-serialization-json` son orientativos. Si Android Studio sugiere otra estable compatible con tu Kotlin, úsala. Detalle de Sync/conflictos: [guía Gradle](./12-gradle-troubleshooting.mdx).
:::

También es interesante configurar Retrofit para que use `kotlinx.serialization` en lugar de Scalars:
