...
1. Anota la versión de **Kotlin** del proyecto.
2. Añade el plugin **KSP** en el proyecto (`apply false`) con una versión **compatible con esa Kotlin** ([releases KSP](https://github.com/google/ksp/releases)).
3. Aplica `id("com.google.devtools.ksp")` en el módulo `app`.
4. Añade Room (`runtime`, `ktx`) y el compiler con `ksp(…)`, preferiblemente vía `libs.versions.toml`.
5. **Sync Now**. Si falla, [guía Gradle](./12-gradle-troubleshooting.mdx) (Kotlin ↔ KSP, kapt vs KSP).

```kotlin title="build.gradle.kts (Project)"
plugins {
    // Ejemplo: KSP 2.0.21-1.0.27 encaja con Kotlin 2.0.21
    // Sustituye por el par Kotlin/KSP de tu wizard
...
KSP analiza las anotaciones de Kotlin (p. ej. `@Entity`, `@Dao`) en tiempo de compilación.

La versión de KSP **debe** ser compatible con la de Kotlin. El número del ejemplo es ilustrativo: el prefijo de KSP (`2.0.21-…`) indica la Kotlin con la que casa.

:::warning[No mezclar kapt y KSP]
Si un tutorial antiguo usa `kapt` para Room, en este curso usa solo `ksp`. Mezclar ambos suele dar errores confusos.
:::
...
- **Preferences DataStore**: Almacena y recupera datos de preferencias clave-valor de forma asíncrona y reactiva.
    ...
    - No se pueden almacenar conjuntos de datos complejos.
...
#### Paso a paso (Preferences DataStore)

1. Añade la dependencia en el módulo `app` (catalog o directa).
2. Sync Project with Gradle Files.
3. Crea el repositorio / extensión `preferencesDataStore` como en los ejemplos siguientes.
4. Si Sync falla: [guía Gradle](./12-gradle-troubleshooting.mdx).

```kotlin
dependencies {
    // Versión orientativa; usa la estable que sugiera el IDE / catalog
    implementation("androidx.datastore:datastore-preferences:1.1.1")
    // o: implementation(libs.androidx.datastore.preferences)
}
```
